classdef RectangularGridNonOrthogonal < RectangularGrid
    ...
    methods
        function obj = RectangularGridNonOrthogonal(shape, alpha, deltas, origins)
            %Constructor method
            %Args:
            ...
            nb_dimensions = length(shape);
            switch nargin
                case 2
                    deltas = ones(1, nb_dimensions);
                    origins = zeros(1, nb_dimensions);
                case 3
                    origins = zeros(1, nb_dimensions);
            end
            obj@RectangularGrid(shape, deltas, origins)
            obj.alpha = alpha;
            obj.cosalpha = cos(alpha);
            obj.sinalpha = sin(alpha);
        end
    ...
    methods 
        function sample = simulate(obj, model, generator)
            ...
            cov = @(h)model.covariance_func(h(2) * obj.deltas(2) *(1 - obj.sinalpha),...
                h(1) * obj.deltas(1) + obj.sinalpha * h(2));
            N2 = obj.N;
            M2 = obj.M;
            generator.turnOn();
    ...
    methods
        function [lags_X, lags_Y] = get_positive_lags_grid(obj)
            ...
            lags_x = (0:N-1) * delta_x * obj.cosalpha;
            lags_y = (0:M-1) * delta_y * obj.sinalpha;
        ...
        function [lags_X, lags_Y] = get_lags_grid(obj)
            ...
            lags_x = (-(obj.N-1):obj.N-1) * obj.delta_x * obj.cosalpha;
            lags_y = (-(obj.M-1):obj.M-1) * obj.delta_y * obj.sinalpha;
            [lags_X, lags_Y] = meshgrid(lags_x, lags_y);
            lags_X = lags_X';
            lags_Y = lags_Y';
        end
        ...
        function [lags_X, lags_Y] = get_lags_grid_(obj, start_, end_)
            ...
            end_x = end_(1);
            end_y = end_(2);
            lags_x = (start_x : end_x) * obj.delta_x * obj.cosalpha;
            lags_y = (start_y : end_y) * obj.delta_y * obj.sinalpha;
            [lags_X, lags_Y] = meshgrid(lags_x, lags_y);
            lags_X = lags_X';
            lags_Y = lags_Y';   
        end

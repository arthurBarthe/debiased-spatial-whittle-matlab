...
initEstimatorFactory.constants = 0;
...
% This defines how much terms we include in aliased Whittle. Here only 1.
whittle_factory.sum_truncation = 4;
...
for i=1:length(nu_values)
    model_family.set_fixed_parameter_values([1 nu_values(i)]);
    disp(['Running simulations for nu = ' num2str(nu_values(i))]);
    %Model with fixed parameters
    model = Model(range_parameter, model_family);
    for j = 1 : length(grid_sizes)
        ...
        for i_sim = 1 : nb_simulations
            ...
            for k = 1 : length(estimators_list)
                ...
                results(i, j, k, i_sim) = estimated_model.getParameters();
...
biases = mean_estimates - range_parameter;
